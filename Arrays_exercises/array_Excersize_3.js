...
/*-----------------------Filter Objects using map() & forEach()-----------------------------------------------------------*/
// Conert objects to a "key Value" array
var newObj = users.map(item => ({ // maps the object USERS to a new object "NEWOBJ"
...
Object.keys(newObj).forEach(key => { // loops through 1 to 10 properties
    console.log(key, newObj[key]);
});
for (var i = 0; i < Object.keys(newObj).length; i++) {
    if (key.item.firstName === 'Gabriella') {
        return ("Gabriella")
    }
}
