...
//    console.log(text);
//});
// filtering with afor loop
var newarr = [];
for (var i = 0; i < users.length; i++) {
    if (users[i].firstName, users[i].lastName) {
        newarr.push(users[i].firstName, users[i].lastName)
    }

}
console.table(newarr);
// using a sorting function for alphabetics//
/*function SortData(users, isAsc) {
    return users.sort((a, b) => (a[firstName] < b[lastName] ? -1 : 1) * (isAsc ? 1 : -1));
